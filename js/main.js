...
if(!target.matches('.cardList > li > button.favorite')) return;
...
$cardList.onclick = e => {
  ...
  if (!e.target.matches('.cardList > li > i.deleteBtn')) return;
...
$sortCompany.addEventListener('click', () => render('company'));
$sortName.addEventListener('click', () => render('name'));
$sortRecent.addEventListener('click', () => render('id'));

// $cardList.addEventListener('click', ({target}) => favoriteList(target));